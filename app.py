...
RULES = {
    'maxDailyLoss': 50, 'maxTrades': 10, 'maxPositionSize': 200,
...
def get_congress_trades():
    ...
    try:
        ...
        trades = []
        text = res.text
        # Parse ticker symbols from the page (format: TICKER:US)
        ticker_pattern = re.findall(r'([A-Z]{1,5}):US.*?(buy|sell)', text, re.IGNORECASE)
        seen = set()
        for ticker, action in ticker_pattern:
            if ticker not in seen and len(ticker) >= 2:
                seen.add(ticker)
                trades.append({'ticker': ticker, 'action': action.lower()})
        return trades[:20]  # limit to 20 most recent
    except Exception as e:
        log_congress(f"Error fetching trades: {str(e)[:50]}")
        return []

def congress_scan():
    """Scan congressional trades and copy buy signals"""
    today = datetime.now(pytz.timezone('America/New_York')).strftime('%Y-%m-%d')
    ...
    if not is_market_hours():
        log_congress("⏰ Outside market hours — will copy trades when market opens")
        return
    ...
    for trade in trades:
        ...
        # Check if already copied this ticker today
        trade_key = f"{today}_{ticker}"
        if trade_key in congress_state['copied_trades']:
            continue

        if bought >= 3:  # max 3 congressional copies per day
            break
...
def manual_congress_scan():
    congress_state['last_scan'] = ''  # reset to force rescan
...
# Auto-start both engines on boot
engine_state['running'] = True
threading.Thread(target=engine_loop, daemon=True).start()
log_scan("🚀 Auto engine started on server boot")
